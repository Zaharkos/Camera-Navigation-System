...
def main():
    ...
    while True:
        ...
        if calibration_stage == 0:
            cv2.putText(frame, f"Step 1: Floor ({len(clicked_points)}/4)", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            for idx, pt in enumerate(clicked_points):
                cv2.circle(frame, pt, 5, (0, 255, 0), -1)
                
        elif calibration_stage == 1:
            cv2.putText(frame, f"Step 2: TAIL -> NOSE ({len(robot_points)}/2)", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2)
